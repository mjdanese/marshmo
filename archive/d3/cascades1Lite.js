function cascades1Lite() {d3.select("svg").remove();
console.log('LITE')
  ...
  d3.selectAll(".rangePath")
    .each(function(d,i){
      transF = d3.select(this)
        .attr("transform", "translate(0 "+ -l*10 +")")
    })
}
    ...
    d3.select(this)
      ...
      .attr("d", d3.line()
        ...
        .y(function(d,i) {
            ...
            varCenter = Math.max(((widthValue/2) - 30*(l/(maxLines*(3/4))) - distCenter)/30,0.2)

            return (
              heightValue*(2/3)) // position at bottom
              + (l*3) + ( -15*varCenter*Math.sin(i/((length-1)/Math.PI))*(y(d)/20) )
          })
...
initialLoops = d3.range(0,4)
for (i=0; i<initialLoops.length; i++){
createLine(i)
}
}

function cascade1() {d3.select("svg").remove();
console.log("FULL")
